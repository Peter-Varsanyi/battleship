...
public class ArtificalIntelligence {
	...
	public ArtificalIntelligence(Board board, String type) throws IOException {
		...
		ShipPossibilityMatrix possibilitymatrix = new ShipPossibilityMatrix();
	...
	private Point getStartingPoint() {
		Point position = null;
		for (Point point : shipPointsFound) {
			if (attackNearPoint(point)) {
				position = point;
			}
		}
	}

	private boolean attackNearPoint(Point point) {
		for (int x = -1; x < 2; x++) {
			for (int y = -1; y < 2; y++) {
				if 
			}
	...
	private Point getNextBestMove() {
		...
		if (bestPoint == null) {
			System.out.println("No idea, lets play random");
			bestPoint = generateNewUniqueRandom();
		} else {
			System.out.println("HAHA I MADE A DECISION");
		}
		lastHit = bestPoint;
		return bestPoint;
	}
